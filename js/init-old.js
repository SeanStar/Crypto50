/* 
 * left menu initialization function
 * this function changes the style of the clicked tab to "active"
 * it also toggles the display of the content coresponding to the clicked tab
 */
initMenu = function(){
...
var plot = $.plot($(id),
           [ { data: players, label: "Players", yaxis: 1 }, { data: ping, label: "Ping", yaxis: 2 } ], {
               series: {
                   lines: { show: true },
                   points: { show: true }
               },
...
initElements = function() {		
	...
	$('.thumbnail a.actions.view').click(function(){		
		$.modal("<div><img src=" + $(this).attr('href') + " /></div>",{onShow: function(dlg) {
		    $(dlg.container).css('height','auto');
		}});
		return false;
	});
